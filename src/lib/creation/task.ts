...
analysis与other均须附加context：{"goal":"本轮目标","mode":"analysis|discussion|revision|general","reportIds":["本轮相关报告ID"],"sourceIds":[],"directionIds":["本轮实际讨论的方向版本ID"],"noteIds":["需回顾的讨论建议ID"]}。从catalog的完整索引选择，不受生成时间限制，不能只取最近报告。已选定方向决定创作基础，不会清除前文依据；结合导语与第一章大概念的请求必须同时选相关报告，候选最初关联报告不是唯一依据。无关报告不选；同份资料同维度有多个报告时，优先sequence较大的最新有效报告，除非用户明确比较版本；已有报告时不重复选只是复述报告的旧助手回复，不把旧失败答复当依据；作者指向选定版本时用selectedDirectionId，指向正在讨论的版本时用focusedDirectionId；提到第二个等需按catalog.candidateBatches中的分组与position消解。未明确旧组时，“继续聊第二个”指向最新生成组的第二个，不能按跨轮扁平列表编号；明确最初/上一组则用对应组。必须将实际方向ID写入context.directionIds，不能留空后猜测。综合答疑用discussion，明确修改候选才用revision。若context不足，可以在下一步用load_context继续取报告、方向或讨论记录。
